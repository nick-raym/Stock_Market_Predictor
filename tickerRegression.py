...
df['Next_Return_1d'] = df['Close'].pct_change().shift(-1)  # kept for daily simulation
...
X = df[FEATURES]
y = df['Next_Return_5d']  # <-- predicting 5-day return
...
try:
    ...
    print(f"\n  5-day predicted return quintiles vs actual 5-day returns:")
...
# Align 1-day actual returns with test set
...
for threshold in [0.0, 0.005, 0.01, 0.015, 0.02, 0.03]:
    for mode in ['long_only', 'long_short', 'long_sit_short']:
        ...
        if mode == 'long_only':
            # Long when pred > threshold, flat otherwise
            ...
            # Scaled: position proportional to predicted return magnitude
        ...
        # Transaction cost: charge when position changes meaningfully
        ...
        trades = (pos_change > 0.05).sum()  # count meaningful position changes
